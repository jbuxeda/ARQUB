"LDAclr"<-
  function(xclr, y = NA, pprevia = 0, grupx = 1, llegendax = grupx, ggrupx = grupx, gllegendax = ggrupx, grupy = 1, llegenday = grupy, ggrupy = grupy, gllegenday = ggrupy, titol1 = 0, titol2 = 0, idioma = 1, paleta = 1, paleta0 = NA, color = "chocolate", simbol = 0, simbol0 = NA, midall = 0.8, nom = NA, lds = 3, colpf = "grey85", tex = 1, cexpunts = 1)
  {
    ...
    # El grup ha de ser un as.factor i la llegenda, si no es el grup, as.character, pero la rutina ja ho vigila. El nom es pel grafic de les funcions discriminants. Per defecte, no n'hi ha (NA).
    ...
    if (idioma == 3) {titoly <- as.character("Variances inter-groupe")}
    if (idioma == 4) {titoly <- as.character("Between-group variances")}
